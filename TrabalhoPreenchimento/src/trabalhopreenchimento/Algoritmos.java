...
public class Algoritmos {
	...
	public static double getAngulo(Point ini, Point fim) {
		...
		int x2 = (int) fim.getX();
		int y2 = (int) fim.getY();
		// O angulo é calculado pelo arcotangente de x,y multiplicado por 180 e
		// divido por pi(para passarmos de radianos para graus)
		double angulo = Math.atan2((x2 - x1), (y2 - y1)) * 180 / Math.PI;
		// Ajuste, o angulo zero é considerado o norte por essa formula,
		// enquanto no nosso uso queremos o leste como angulo zero
		angulo -= 90;
	...
	public BufferedImage desenhaRetaBresenham(BufferedImage img, Point ini, Point fim) {
		...
		// Bresenham
		// Considerando um circulo trigonometrico invertido(o java mapeia a
		// imagem de baixo pra cima, então o algoritmo funciona de 315 a 360
		// graus)
		angulo = getAngulo(ini, fim);
		/*
		 * Octantes: \5|6/ 4\|/7 ---+--- 3/|\0 /2|1\
		 * 
		 */

		// O bresenham só funciona normalmente no primeiro octante, todas as
		// outras soluções requerem adaptações
	...
	private BufferedImage bresenhamOct0(BufferedImage img, int xini, int yini, int xfim, int yfim) {
		...
		// Definimos um valor inicial para o erro, ele será corrigido a cada
		// laço
		double erro = -1.0;
		...
		for (tmpx = xini; tmpx < xfim; tmpx++) {
			...
			erro = erro + deltaerr;
			// Quando o erro se torna positivo, é sinal que devemos deslocar um
			// pixel na direção y
			if (erro >= 0.0) {
				// desloca o pixel
				tmpy++;
	...
	private BufferedImage bresenhamOct1(BufferedImage img, int xini, int yini, int xfim, int yfim) {
		...
		// Será necessário inverter algumas veriaveis para que a linha base seja
		// o eixo y e o algoritmo funcione
		deltax = xfim - xini;
		...
		// Nota-se que o comportamento deles são quase iguais, não fica na mesma
		// função, mas alterando os parametros
		// Devido a essa inversão gerar problemas no desenho
		tmpx = xini;
		for (tmpy = yini; tmpy < yfim ; tmpy++) {
			img.setRGB(tmpx, tmpy, cor.getRGB());
			erro = erro + deltaerr;
			if (erro >= 0.0) {
				tmpx++;
	...
	public BufferedImage preencheVarredura(BufferedImage img, int bbox_cima, int bbox_baixo, int bbox_esquerda,
			int bbox_direita) {
		int pinta = 0;

		for (int i = bbox_esquerda; i < bbox_direita; i++) {
			pinta = 0;
			for (int j = bbox_cima; j < bbox_baixo; j++) {
	...
	public BufferedImage preencheFlood(BufferedImage img, int x, int y) {
		...
		if(img.getRGB(x, y) != -16777216){
			...
			//Linha superior ao ponto
			preencheFlood(img, x-1, y-1);
			preencheFlood(img, x, y-1);
			preencheFlood(img, x+1, y-1);
			//Linha do ponto
			preencheFlood(img, x-1, y);
			//preencheFlood(img, x, y-1);
			preencheFlood(img, x+1, y);
			//linha inferior
			preencheFlood(img, x-1, y+1);
